$(document).ready(function(){
    ...
    $(".btn1").click(function(){
        gsap.to('.target1', 1, { x: 450});
        gsap.to('.target1', 1, { x: 0});
        gsap.to('.target1', 1, { x: 225, y: 225});
        gsap.to('.target1', 3, { rotation: 360});

    });
    $(".btn2").click(function(){
        gsap.to('.target1', 3, { rotation: -360});
        gsap.to('.target1', 1, { x: -25, y: -5});
        gsap.to('.target1', 1, { x: 0});
        gsap.to('.target1', 1, { x: -50});

    });
    
    $(".btn3").click(function(){ 
        gsap.fromTo('.target1',1, { x:30,},{x:300});   
    });

    var tl = gsap.timeline();
    $(".btn4").click(function(){ 
        tl.to(".target1", {duration: 0.5, x: 100, opacity: 0.9})
        .to(".target1", {duration: 0.5, y: 200})
  .to(".target1", {duration: 0.5, rotation: 360}).repeat(3).yoyo( true );
    });
    ...
    t2.to(".target2", 1, { backgroundColor: "cornflowerblue"})
          .to(".target2", 3, {
            x: $(window).width() -400- $(".target2").width(),rotation:360
          },0.5).repeat(1).yoyo(true);

    t3.to(".target3", 1, { backgroundColor: "blue"})
          .to(".target3", 1, {
            x: $(window).width() -200- $(".target3").width(),scale:1.5
          }, 0).repeat(1).yoyo(1);
